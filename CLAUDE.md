...
2. **The deny set is not a security boundary and must not be sold as one.** The
   ...
   door; `NODE_OPTIONS`, `BASH_ENV`/`ENV` and `IFS` the same for what the
   session then spawns. A longer list aimed at "dangerous" could only ever be
   the variables somebody remembered — every interpreter ships one.
...
4. **Order is the safety property, in both places, and both were already
   ...
   silently overridden is worse than one that is refused.
...
7. **Caps, because this is the first field on the route that invites a blob.**
   `readJson` there is uncapped, which never mattered for a few hundred bytes of
   control payload; `env` reaches `execve` and the index file. Capped in the
   sanitizer rather than by swapping the route's reader, so the autonomous
   `input` prompt keeps its own generous length.
